...
export default function ManageReport() {
  ...
  async function BlockUser(id,block){
    const resData = await postData('/admin/updateUser', { id: id , blockedByAdmin:block});
    if (resData && resData.code == 1) {
      fetchData(pageNumber,pageSize)
      if(block)
      success("User Blocked");
      else
      success("User Un-Blocked");
    } else {
      console.log(resData)
                                      ...
                                      return(
                                        ...
                                        <td>{(index + 1+(pageSize*(pageNumber-1)))}</td>
                                          <td>
                                            ...
                                            />{" "}
                                            <Link
                                              to={{ pathname: "/Profile-Details", state: { id: ele.user.id } }}
                                              className="user-name"
                                            >
                                              {" "}
                                              {ele.user.username}
                                          ...
                                          <td>
                                            {ele.post &&
                                              <Link to={{pathname:"/Post-Details",state:{id:ele.post.user.id}}} > Open Posts</Link>
                                            }
                                          </td>
                                          <td>
                                            <img
                                              className="setting-avatar"
                                              src={ele.post && ele.post.user.displayPicture}
                                            />{" "}
                                            {ele.post && ele.post.user.username}
                                          </td>
                                          <td>{ele.reportText}</td>
                                          <td>{moment(ele.createdAt).format('DD-MM-YY hh:mmA')}</td>
                                          <td>
                                            {ele.post && ele.post.user.blockedByAdmin && 
                                              <button
                                                className="btn btn-primary"
                                                onClick={e=>{BlockUser(ele.post && ele.post.user.id,false)}}
                                              >
                                                Un-Block User
                                              </button>
                                            }
                                            {ele.post && !ele.post.user.blockedByAdmin &&  
                                              <button
                                                ...
                                                onClick={e=>{BlockUser(ele.post && ele.post.user.id,true)}}
                                              >
                                                Block User
                                              </button>
                                            }
                            ...
                            <div style={{width:"100%"}}>
                              {
                                ...
                                (
                                  <>
                                  <a onClick={(e) =>{fetchData(1,pageSize)}} href={ pageNumber>1 &&("#")}>{"  "}First</a>
                              ...
                              <span>  
                                 {
                                   pageAnchor.map((ele,index)=>{
                                     if(ele===pageNumber){
                                      ...
                                      return (
                                        <a 
                                          onClick={(e) =>{fetchData(ele,pageSize)}} 
                                          href="#"
                                        >
                                          {ele+"    "}
                                        </a>
                              ...
                              </span>
                              {
                                ...
                                (
                                  <>
                                  <a onClick={(e) =>{fetchData(pageNumber+1,pageSize)}} href={ pageNumber>1 &&("#")}>{"  "}&gt;&gt;</a>
                                  <a onClick={(e) =>{fetchData(lastPage,pageSize)}} href={ pageNumber>1 &&("#")}>{"  "}Last</a>
                                  </>
                                ):(
                                  <a></a>
                                )
